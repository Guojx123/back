          ...
          Container(
            ...
            child: CircleAnimLayout(
              children: List.generate(8, (_) {})
                  .map((v) => CircleAvatar(child: Icon(Icons.attach_money)))
                  .toList(),
  ...
  void paintChildren(FlowPaintingContext context) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      context.paintChild(i,
          transform: Matrix4.translationValues(offsetX - width, offsetY - height, 0.0));
    }
  }
...
class _CircleAnimLayoutState extends State<CircleAnimLayout> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    controller = AnimationController(duration: Duration(milliseconds: 1000), vsync: this)
      ..addListener(
          () => setState(() => rad = (closed ? (controller.value) : 1 - controller.value)))
...
class CircleAnimLayoutDelegate extends FlowDelegate {
  ...
  void paintChildren(FlowPaintingContext context) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      context.paintChild(i,
          transform: Matrix4.translationValues(offsetX - cSizeX, offsetY - cSizeY, 0.0));
    }
